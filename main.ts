...
interface ComponentInfo {
  ...
  componentSetId?: string; // ID of the ComponentSet this variant belongs to (if it's a variant)
  componentSetName?: string; // Name of the ComponentSet this variant belongs to (if it's a variant)
  variantProperties?: { [key: string]: string }; // Variant properties for this component
}
...
figma.ui.onmessage = async (msg) => {
  if (msg.type === 'show-published') {
    const publishedComponents = await getPublishedComponents();
    
    if (publishedComponents.length > 0) {
      try {
        await createTextFieldOnStage(publishedComponents);
        figma.ui.postMessage({ 
          type: 'enable-buttons',
