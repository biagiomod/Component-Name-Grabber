...
function extractVariantGroups(variants) {
    const groupsMap = {};
        ...
        for (const propertyName in variant.variantProperties) {
            if (variant.variantProperties.hasOwnProperty(propertyName)) {
                ...
                if (!groupsMap[propertyName]) {
                    groupsMap[propertyName] = new Set();
                }
...
function getComponentSetInfo(componentSet, variants) {
    return __awaiter(this, void 0, void 0, function* () {
        const entry = {
                ...
                if (componentSet.componentPropertyDefinitions.hasOwnProperty(propName)) {
                    ...
                    if ('defaultValue' in propDef && propDef.defaultValue !== undefined) {
                        if (propDef.type === 'BOOLEAN') {
                            componentProp.defaultValue = propDef.defaultValue;
                        }
...
function getComponentSetIdFromComponent(componentNode) {
    try {
...
function extractInstancePropertyValues(instance) {
    const properties = {};
        ...
        if (instance.componentProperties.hasOwnProperty(propName)) {
            try {
                ...
                if (prop.type === 'TEXT') {
                    ...
                    const swappedComponent = prop.value;
...
function extractNestedInstances(node_1) {
    return __awaiter(this, arguments, void 0, function* (node, depth = 0, maxDepth = 10) {
        ...
        for (const child of node.children) {
            if (child.type === 'INSTANCE') {
                ...
                const deeperNested = yield extractNestedInstances(instance, depth + 1, maxDepth);
                ...
                const deeperNested = yield extractNestedInstances(child, depth + 1, maxDepth);
...
function extractInstanceData(instance) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const nestedInstances = yield extractNestedInstances(instance);
...
function gatherAllInstances() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const instancesByComponentSet = new Map();
            for (const instance of allInstances) {
                try {
                    ...
                    const instanceData = yield extractInstanceData(instance);
...
function getPublishedComponentsHierarchical() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            // Send status update
            figma.ui.postMessage({
            ...
            for (let i = 0; i < allNodes.length; i++) {
                ...
                if (node.type === 'COMPONENT_SET') {
                    ...
                    try {
                        ...
                        const variantNodes = componentSet.children.filter(child => child.type === 'COMPONENT');
                        ...
                        for (const variantNode of variantNodes) {
                            ...
                            if (variantNode.key) {
                                try {
                                    const variantInfo = yield getVariantInfo(variantNode);
                        ...
                        if (variants.length > 0) {
                            const componentSetEntry = yield getComponentSetInfo(componentSet, variants);
            ...
            for (const standalone of standaloneComponents) {
                ...
                try {
                    const variantInfo = yield getVariantInfo(standalone);
            ...
            const instancesByComponent = yield gatherAllInstances();
            ...
            const metadataMap = yield scanMetadataFromStageNotes();
...
function getSelectedOrPublishedComponentSets() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            // Check if user has selected ComponentSet nodes
            const selection = figma.currentPage.selection;
            const selectedComponentSets = [];
            for (const node of selection) {
                if (node.type === 'COMPONENT_SET') {
                    selectedComponentSets.push(node);
                }
            }
            if (selectedComponentSets.length > 0) {
                ...
                for (const componentSet of selectedComponentSets) {
                    ...
                    if (variants.length > 0) {
                        const entry = yield getComponentSetInfo(componentSet, variants);
                        entries.push(entry);
                    }
                }
                return entries;
                ...
                return yield getPublishedComponentsHierarchical();
            }
        }
        catch (error) {
...
function organizeVariantsByProperties(componentSetEntry) {
...
function createPropertyChip(propertyName, propertyValue) {
...
function createVariantThumbnail(variant) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            // Find the variant component node
            const variantNode = yield figma.getNodeByIdAsync(variant.id);
...
function findNoteFrame(componentName) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            yield figma.loadAllPagesAsync();
            const noteFrames = figma.root.findAll(node => node.type === 'FRAME' && node.name === `NOTE_${componentName}`);
...
function createComponentOutlineFrame(componentSetEntry) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            // Load fonts
            ...
            if (yAxisProperty || xAxisProperty) {
                ...
                if (yAxisProperty)
                    outlineInfo += `Y: ${yAxisProperty}`;
                if (xAxisProperty)
                    outlineInfo += `, X: ${xAxisProperty}`;
                ...
                for (const [yValue, xMap] of hierarchy.entries()) {
                    ...
                    for (const [xValue, variants] of xMap.entries()) {
                        ...
                        for (const variant of variants) {
                            ...
                            // Optional thumbnail
                            const thumbnail = yield createVariantThumbnail(variant);
                            if (thumbnail) {
            ...
            // Position frame
            const noteFrame = yield findNoteFrame(componentSetEntry.name);
            if (noteFrame) {
...
function renderComponentOutlines() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            figma.ui.postMessage({
                type: 'status-update',
            ...
            const componentSets = yield getSelectedOrPublishedComponentSets();
            ...
            for (let i = 0; i < componentSets.length; i++) {
                ...
                const frame = yield createComponentOutlineFrame(componentSet);
                if (frame) {
...
function scanMetadataFromStageNotes() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            yield figma.loadAllPagesAsync();
            // Find all frames with names starting with "NOTE_"
            const allFrames = figma.root.findAll(node => node.type === 'FRAME' && node.name.startsWith('NOTE_'));
            ...
            for (const frame of allFrames) {
                try {
                    ...
                    const metadataFrame = frame.children.find(child => child.type === 'FRAME' && child.name === 'Metadata');
                    if (!metadataFrame) {
                    ...
                    for (const fieldName of fieldNames) {
                        const fieldFrame = metadataFrame.children.find(child => child.type === 'FRAME' && child.name === fieldName);
                        if (fieldFrame) {
                            // Find the TextNode (the editable text field - second child after label)
                            const textNodes = fieldFrame.children.filter(child => child.type === 'TEXT');
...
function createMetadataFrame(componentSetEntry) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            // Find the component node in the document
            const componentNode = yield findNodeById(componentSetEntry.id);
            if (!componentNode) {
...
function createMetadataFrames() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const componentSetEntries = yield getPublishedComponentsHierarchical();
            if (componentSetEntries.length === 0) {
            ...
            for (let i = 0; i < componentSetEntries.length; i++) {
                ...
                const frame = yield createMetadataFrame(entry);
                if (frame) {
                    createdFrames.push(frame);
                    successCount++;
                }
